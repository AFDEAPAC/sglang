        ...
        if quant_config._is_wNa16_group_channel(weight_quant, input_quant):
            if not _is_npu:
                if (
                    ...
                    # Try aiter/FlyDSL for W4A16 (per-row scale or group_size=32)
                    _w4a16_config = quant_config.target_scheme_map["Linear"].get("weights")
                    ...
                    if _use_aiter and _is_w4a16 and _is_flydsl_compatible:
                        try:
                            logger.info_once(
                                "Using CompressedTensorsWNA16AiterMoEMethod (ROCm + FlyDSL)"
                            )
                            return CompressedTensorsWNA16AiterMoEMethod(quant_config)
                        except ValueError as e:
                            logger.warning(f"FlyDSL W4A16 not available: {e}, falling back to Triton")
...
class CompressedTensorsWNA16AiterMoEMethod(CompressedTensorsWNA16MoEMethod):
    ...
    def __init__(self, quant_config, num_gpu_experts=-1):
        ...
        # FlyDSL W4A16 supports per-row scale or group_size=32
        ...
        # Ensure this is W4A16 (4-bit weights)
        if self.num_bits != 4:
            raise ValueError(
    ...
    def apply(
        ...
        assert self.moe_runner_config.activation == "silu", "FlyDSL W4A16 only supports SiLU"
