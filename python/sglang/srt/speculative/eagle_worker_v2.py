...
class EagleDraftWorker(BaseDraftWorker):
    ...
    def _draft_fast_topk(
        self, logits: torch.Tensor
    ) -> "tuple[torch.Tensor, torch.Tensor]":
        """Select draft tokens from logits.

        When eagle-topk=1 the softmax is unnecessary: argmax is invariant to
        monotone transforms, and topk_p is unused for candidate selection in a
        single-path (degenerate) tree.  Skipping softmax saves V=151k exp()
        calls per draft step.
        """
        if self.topk == 1:
            topk_index = torch.argmax(logits, dim=-1, keepdim=True)
            topk_p = torch.ones(
                topk_index.shape, dtype=torch.float32, device=logits.device
            )
    ...
    def draft_forward(self, forward_batch: ForwardBatch):
        ...
        for i in range(self.speculative_num_steps):
            ...
            topk_p, topk_index = self._draft_fast_topk(
    ...
    def _draft_extend_for_prefill(
        ...
        next_draft_input.topk_p, next_draft_input.topk_index = (
            self._draft_fast_topk(logits_output.next_token_logits)
        )
    ...
    def _draft_extend_for_decode(
        ...
        ret_topk_p, ret_topk_index = self._draft_fast_topk(
